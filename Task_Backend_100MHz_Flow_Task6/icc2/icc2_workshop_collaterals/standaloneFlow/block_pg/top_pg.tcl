...
set_app_options -name plan.pgroute.discard_via_outside_design -value true
set_app_options -name plan.pgroute.discard_via_origin_outside_design -value true
# mask constraints
# via master rules
# PG patterns
...
# PG strategies
set_pg_strategy core_ring -core  -pattern {{pattern: ring_pattern}{nets: {VDD VSS}}{offset: {3 3}}} -blockage {{blocks : all}} -extension {{stop: innermost_ring}} 
set_pg_strategy s_mesh1 -core  -pattern {{pattern: pg_mesh1} {nets: {VDD VSS VSS VDD}}  {offset_start: 10 20} {parameters: 4 80 6 120 3.344 false}} -blockage {{blocks : all}} -extension {{stop: outermost_ring}} 
